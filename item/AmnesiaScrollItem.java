...
public class AmnesiaScrollItem extends Item {
   ...
   public void appendHoverText(@NotNull ItemStack itemStack, Level level, List<Component> components, @NotNull TooltipFlag tooltipFlag) {
      components.add(Component.translatable(this.getDescriptionId() + ".tooltip").withStyle(ChatFormatting.GOLD));
      double penalty = ServerConfig.amnesia_scroll_penalty;
      if (penalty > 0.0) {
         int textPenalty = (int)(penalty * 100.0);
         components.add(Component.translatable(this.getDescriptionId() + ".warning", new Object[]{textPenalty}).withStyle(ChatFormatting.RED));
      }
   }
}
